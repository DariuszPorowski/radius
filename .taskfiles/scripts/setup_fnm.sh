...
"${INSTALL_DIR}/${TOOL_NAME}" --version || die "Installed binary failed to run (${INSTALL_DIR}/fnm)"
...
add_to_shell() {
  ...
  if [[ -f "${shell_config}" ]]; then
    ...
    if ! grep -q "fnm env" "${shell_config}"; then
      echo "eval \"\$(fnm env --use-on-cd --version-file-strategy=recursive --resolve-engines --shell ${shell_type})\"" >>"${shell_config}"
    fi
  fi
}
